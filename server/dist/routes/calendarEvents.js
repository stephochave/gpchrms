...
const calendarEventSchema = zod_1.z.object({
    ...
    type: zod_1.z.enum(['reminder', 'event']).default('reminder'),
    description: zod_1.z.string().optional().nullable(),
    eventDate: zod_1.z.string().min(1, 'Event date is required'),
    eventTime: zod_1.z.string().optional().nullable(),
...
router.get('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const [rows] = await db_1.pool.execute('SELECT id, title, type, description, event_date, event_time, created_by, created_at, updated_at FROM calendar_events WHERE id = ?', [id]);
        if (rows.length === 0) {
...
router.post('/', async (req, res) => {
    ...
    try {
        const [result] = await db_1.pool.execute('INSERT INTO calendar_events (title, type, description, event_date, event_time, created_by) VALUES (?, ?, ?, ?, ?, ?)', [title, type, description || null, eventDate, eventTime || null, createdBy || null]);
        const insertId = result.insertId;
        const [rows] = await db_1.pool.execute('SELECT id, title, type, description, event_date, event_time, created_by, created_at, updated_at FROM calendar_events WHERE id = ?', [insertId]);
        // Log activity
        ...
        if (type === 'event') {
            try {
                ...
                await db_1.pool.execute('INSERT INTO notifications (title, description, type, related_id) VALUES (?, ?, ?, ?)', [title, notificationDescription, 'event', String(insertId)]);
            }
            catch (notifError) {
...
router.put('/:id', async (req, res) => {
    ...
    try {
        ...
        await db_1.pool.execute(`UPDATE calendar_events SET ${updateFields.join(', ')} WHERE id = ?`, updateValues);
        const [rows] = await db_1.pool.execute('SELECT id, title, type, description, event_date, event_time, created_by, created_at, updated_at FROM calendar_events WHERE id = ?', [id]);
        if (rows.length === 0) {
            return res.status(404).json({ message: 'Calendar event not found' });
        }
...
router.delete('/:id', async (req, res) => {
    ...
    try {
        ...
        const [eventRows] = await db_1.pool.execute('SELECT title, type, created_by FROM calendar_events WHERE id = ?', [id]);
        const event = eventRows[0];
        const [result] = await db_1.pool.execute('DELETE FROM calendar_events WHERE id = ?', [id]);
