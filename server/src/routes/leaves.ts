  ...
  try {
    ...
    const [result] = await pool.execute(
      `INSERT INTO leave_requests 
         (employee_id, employee_name, employee_department, leave_type, start_date, end_date, reason, status, total_days)
       VALUES (?, ?, ?, ?, ?, ?, ?, 'pending', DATEDIFF(?, ?))`,
      [employeeId, employeeName, employeeDepartment, leaveType, startDate, endDate, reason, endDate, startDate],
    );
